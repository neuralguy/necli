...
def _print_help() -> None:
    from commands.registry import by_category, command_label
    ...
    for _cat, cat_key, cmds in groups:
        ...
        for c in cmds:
            label = command_label(c.name, include_action=False)
            padding = " " * (col_width - len(label))
            desc = _(c.desc_key).rstrip().rstrip(".。!;")
            aliases = ""
            if c.aliases:
...
async def _handle_slash(
    ...
    if head == "/models":
        ...
        def _append_router_manager(label: str = "routers") -> None:
            api_models.append(
                ApiModelInfo(
                    id="__manage_routers__",
                    display_name=_("routers.manage"),
                    context_window=0,
                )
            )
            model_providers.append("__router_manager__")
        ...
        if choice is not None:
            ...
            if chosen_provider == "__router_manager__":
                from commands.menus.routers import routers_interactive

                await _call_menu(routers_interactive)
                return r
    ...
    if head == "/sessions":
        sessions_list = await asyncio.to_thread(storage.list_sessions, 0, True)
    ...
    if head == "/stats":
        ...
        await _call_menu(stats_interactive, session, period_days, model)
        return r
    ...
    if head == "/tools":
        from commands.menus.tools import tools_interactive

        await _call_menu(tools_interactive)
        return r
