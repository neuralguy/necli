...
через инструмент `skill`. Для model-invoked скиллов активность вычисляется из
истории сообщений: скилл активен, если его последняя загрузка попала в окно
последних `ACTIVE_WINDOW_ROUNDS` раундов (раунд = одно сообщение пользователя).
Явно включённые через интерактивное меню скиллы хранятся в skills.manager и
добавляются к этому множеству в apis.agent_adapter.current_active_skills().

Оконная активность синхронизирует три механизма:
...
def active_skills_from_messages(
    messages: list,
    window: int = ACTIVE_WINDOW_ROUNDS,
) -> set[str]:
