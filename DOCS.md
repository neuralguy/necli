...
│                                 # lang, lsp, mcp, memory, params, permissions, prompt,
│                                 # proxy, settings, skills, stats, telegram, themes, tools,
│                                 # ui_customization, _editor, _style
│
...
├── ui.json                    # override символов/лейблов/цветов инструментов + лимиты
...
Пользовательские ролевые инструкции редактируются через `/settings` → Model → Model prompt и хранятся в `.data/user_prompt.md` (или в каталоге `NECLI_HOME`). В меню можно посмотреть первые пять строк, включить или отключить подмешивание и открыть файл в `$EDITOR`. При включённой настройке непустое содержимое заменяет встроенный ролевой блок в начале системного промпта; пустой файл или отключённая настройка оставляют встроенную роль. В `/settings` → Memory доступны переключатель памяти, просмотр записей, редактирование через `$EDITOR` (по умолчанию `nano`) и удаление записей проекта/глобальной памяти.
...
| `stream_idle_timeout` | Максимальное ожидание следующего stream-чанка, 1–3600 секунд (дефолт `180`). Меняется через `/settings` → Model → Parameters. |
...
`apis/chatgpt_auth.py` реализует OAuth 2.0 Authorization Code + PKCE. Вход расположен отдельной первой строкой главного меню `/api`, а не внутри добавления пользовательского провайдера. ChatGPT исключается из общего списка: после входа эта же верхняя строка становится `ChatGPT OAuth` и открывает сокращённое меню «использовать / модели / выйти». При каждой новой попытке прежний callback этого процесса принудительно останавливается до повторного bind на `127.0.0.1:1455`. Браузер открывает `auth.openai.com`, callback проверяет `state`, после чего access/refresh token сохраняются отдельно. Access token обновляется заранее и повторно после первого HTTP 401. `apis/providers/chatgpt_provider.py` преобразует внутренние сообщения и native function tools в Responses input items, передаёт `ChatGPT-Account-Id` и по умолчанию стримит каждый ответ отдельным SSE-запросом. Экспериментальный постоянный WebSocket можно включить только явно полем `extra.websocket: true` в определении провайдера; успешный `response.completed` тогда сохраняет `previous_response_id`, и при обычном продолжении той же истории следующий `response.create` содержит только новые input items. Продолжения с `function_call_output` отправляются с полной историей вызова: backend иногда теряет связь tool call за `previous_response_id`, что иначе приводит к HTTP 400. Повреждённый или дублированный tool output без соответствующего вызова сохраняется как пользовательский контекст вместо невалидного Responses item. `response.incomplete` считается штатным терминальным событием и передаёт причину ограничения агентному циклу. При ошибке открытия экспериментального WebSocket до начала ответа провайдер переключается на SSE-транспорт, а HTTP 401 во время handshake один раз обновляет OAuth-токен. Оба транспорта используют единый преобразователь Responses events в `AIMessageChunk`, проверяют наличие терминального события и классифицируют временные HTTP/Responses-ошибки для retry. Встроенные Sol, Terra и Luna имеют официальный контекст 1,05 млн токенов и стандартные short-context API-цены за 1 млн токенов; это справочная оценка, поскольку OAuth-запросы расходуют лимит подписки, а не оплачиваются как API-вызовы. `apis/chatgpt_usage.py` читает subscription rate limits из `backend-api/wham/usage`, определяет недельное окно по его длительности и кеширует оставшийся процент. Обновление выполняется в фоне при запуске и после открытия `/api` или `/models`, не задерживая первый рендер меню, а также после каждого завершённого ответа. Один кеш используют верхняя OAuth-строка `/api`, секция ChatGPT в `/models` и строка статуса над вводом; подписка UI на обновления перерисовывает статус после завершения фонового запроса.

Общий stream-wrapper `apis/_retry.py` контролирует все провайдеры одинаково: ожидание нового `AIMessageChunk` ограничено настройкой `stream_idle_timeout` (180 секунд по умолчанию), чтобы краткая пауза reasoning не принималась за сетевой сбой. Пустой зависший запрос повторяется один раз; частичный зависший запрос также получает одну попытку восстановления. EOF без provider-specific terminal event и временные транспортные ошибки повторяются с backoff. Повторный стрим не откатывает уже показанный текст; если все попытки оборвались после получения данных, wrapper возвращает самый длинный частичный ответ как incomplete, и агентный цикл продолжает его вместо потери результата и немедленного `API connection error`. Незавершённый native tool call отбрасывается, чтобы не допустить выполнения повреждённых аргументов. OpenAI-compatible, ChatGPT Responses, Anthropic и Google явно выставляют terminal/incomplete metadata по собственным событиям протокола. У ChatGPT досрочное закрытие response дополнительно закрывает постоянный WebSocket: retry никогда не переиспользует соединение с недочитанным ответом.
...
`/helpers` хранит независимые пары `provider/model` для helper- и image-запросов. Helper-модель обслуживает изолированные one-shot операции (`compress`, recap, извлечение памяти, insights), не меняя основную `ApiSession`; compression использует тот же streaming transport и retry/idle-timeout, что основной ответ, поэтому не зависит от менее надёжного отдельного non-stream запроса. Изображение сначала отправляется основной модели. Если основной запрос с изображением завершается ошибкой либо успешный ответ явно показывает, что шлюз не передал модели вложение (модель сообщает о недоступности изображения или пытается прочитать исходный файл через tool), оригинальное изображение отправляется настроенной image-модели, после чего основной запрос один раз повторяется с подробным текстовым блоком `<image_descriptions>`. Это одинаково работает для изображений из буфера и результатов tools. Описание сохраняется рядом с attachment и восстанавливается как текст после resume. Пустая или ставшая невалидной настройка image-модели оставляет прежний text-only fallback с пометкой об ошибке чтения фото.
...
Каждая модель: `id`, `display_name`, `context_window`, `input_price` / `output_price` (USD за 1M токенов). Если `context_window` не задан (нет в JSON-определении, не пришёл из автообнаружения, не введён в меню) — используется единый дефолт `models.DEFAULT_CONTEXT_LIMIT` (1M токенов) — единственный источник истины для `ApiModelInfo`, `apis/registry.py`, `apis/config.py`, `apis/model_discovery.py`, меню `/api`, статус-бара и `/stats`; он же служит fallback-лимитом в `models.get_context_limit` для модей, которых нет в конфигах. При первой загрузке конфига старое неявное значение 128K автоматически мигрирует на 1M только у моделей без отдельного display name; явные лимиты именованных модей не меняются. Цены используются в `session/session.py:_compute_cost`: при наличии реального `usage` от провайдера — он, иначе fallback на tiktoken-оценку. `cache_read` вычитается из обычного input и тарифицируется с коэффициентом провайдера (`cache_read_factor`, по умолчанию `0.1`); коэффициент и эквивалент оплачиваемых cached-токенов сохраняются в статистике сессии, чтобы пересчёт по новым ценам не терял скидку.
...
- Число запросов к модели (`⟳`), вызовов инструментов (`⚒︎`), токены в порядке `c<cache> ↑input ↓output`.
...
- `background=true` в аргументах `shell` запускает команду в daemon-потоке без доступа к вводу терминала.
...
- Потоковый stdout/stderr читается бинарными чанками до 64 KiB и накапливается в ограниченном head/tail-буфере без lock на каждую строку. Результат хранит до 1 000 000 символов с явной пометкой пропуска, а live-оверлей показывает последние 50 000 символов.
- Частый вывод не создаёт callback на каждую строку: обычная строка и открытый оверлей обновляются общим UI ticker, финальное изменение дополнительно проходит через throttled thread-safe invalidation.
- `_bg_pump` очищает finish-event до постановки `SUBMIT_BG_RESUME` в очередь. Это гарантирует возврат управления UI loop и не позволяет завершившейся задаче запустить бесконечный цикл `event.wait()`/`put_nowait()`.
...
- Текст перед native tool call сразу финализируется в scrollback; пока JSON аргументов дописывается, динамическая зона показывает имя инструмента и уже полученные ключевые аргументы, а файловые пути внутри рабочего каталога сокращает до относительных.
...
- Первый раунд `agent/messages.build_first_message` добавляет незавершённый план и сессионные заметки. Динамические plan/external блоки следующих раундов доставляются request-local и не сохраняются в `ApiSession`; external-блок дополнительно удаляется из fenced history, поэтому одноразовые изменения не мутируют повторно используемый prompt-cache prefix.
...
- Токены везде рисуются в порядке `c<cache> ↑input ↓output` (Working, статус-строка, replay, headless-сводка).
...
- Незавершённые или невалидные `args` после стрима не исполняются: повторный non-stream запрос не делается, чтобы не удваивать расход.
...
- **`patch_file`** — атомарный массив `patches` с парами `find/replace`; одиночная форма не поддерживается. В UI общий отступ многострочных FIND/REPLACE-блоков и применённого diff убирается только при отображении; при fuzzy-матчинге различия внешнего отступа учитываются, а вложенные отступы в файле сохраняются. Fuzzy-матч с warning'ом (`tools/file_ops/_fuzzy.py`).
...
- **`shell`** — запускается из рабочей директории агента; `cd` и shell-композиция (`&&`, `||`, pipelines) действуют внутри одного вызова. Тяжёлые/долгие команды можно запускать в фоне: `background=true` в аргументах (`tools/background.py`) — задача исполняется отдельно, результат приходит уведомлением по завершении.
...
- **`image_search`** — поиск картинок в сети и скачивание их в `assets/images`. Файлы именуются по поисковому запросу с заменой пробелов на `_` и последовательным номером (`поисковый_запрос_1`, `поисковый_запрос_2`, …); повторный одинаковый запрос продолжает существующую нумерацию без перезаписи файлов.
...
`_compute_cost` приоритетно использует реальный `usage` от провайдера (`input`, `output`, `reasoning`, `cache_read`). Если usage нет — fallback на tiktoken через буфер input-сообщений. Обычный input оплачивается полностью, `cache_read` — с сохранённым коэффициентом провайдера (`0.1` по умолчанию), output — по output-тарифу. Та же формула используется в `/stats`, периодной статистике и при пересчёте сохранённых сессий.
...
- `compress_reset` сериализует историю, отправляет helper-модели (или основной как fallback) через streaming `api_compress_history`, кладёт сжатый текст обратно как `system` сообщения;
...
| `/settings` | misc | Группированные настройки: helper/image-модель и params, пользовательская роль, память (вкл/выкл и CRUD записей), MCP, LSP, язык. |
...
- Контекстная подмена workdir — через `ContextVar` в `tools/_paths.py` (`use_working_dir(path)`). `resolve_path` использует `os.path.normpath`, **не `realpath`** — чтобы симлинки `.venv` / `node_modules` внутри worktree разрешались в свои кэши, но запись не утекала по симлинкам. Shell-команды стартуют с этим cwd; если в корне есть `.venv` или `venv`, дочернему процессу автоматически выставляются `VIRTUAL_ENV` и соответствующий `bin`/`Scripts` в начале `PATH`.
...
1. **Инжекция в промпт** — если `memory_enabled=true`, `format_memory_block()` добавляет закреплённые/приоритетные записи и до 8 записей, лексически релевантных текущему пользовательскому запросу, в блок `<persistent_memory>` (`system_prompt._build_memory_block`, лимит ~6000 символов). Порядок файлов больше не определяет, какая память попадёт в контекст. При `memory_enabled=false` блок не добавляется.
...
3. **Ручное редактирование моделью** — единый инструмент `memory` с действиями write / list / read / delete (`tools/memory_tool.py`): только основной агент по ходу текущего диалога решает, когда сохранить или удалить долговременный факт. При `memory_enabled=false` схема не передаётся модели, а прямой вызов возвращает сообщение «Инструмент памяти недоступен: память отключена в /settings».
...
- `Ctrl+P` — вставить изображение из буфера: сохраняется в `.data/clipboard_images/`, в тексте — плейсхолдер `[imageN]`. Сначала изображение отправляется основной модели; только при ошибке запроса настроенная image-модель описывает оригинал, после чего основной модели передаётся текстовый блок `<image_descriptions>`. Без image-модели используется text-only fallback с пометкой об ошибке чтения фото.
...
`ui/focus.py` — отслеживание фокуса терминала через focus reporting (`CSI ?1004`): обёртка над `Vt100Input` вырезает `\x1b[I` / `\x1b[O]` из потока ввода до vt100-парсера (prompt_toolkit их не знает) и превращает в булево состояние (`is_terminal_focused()`, `None` = терминал не отчитывается). Включение привязано к raw-режиму ввода (`bind_focus_reporting_to_raw_mode` оборачивает `Input.raw_mode()` / `Input.cooked_mode()`): `\x1b[?1004h` пишется только после перехода tty в raw и снимается (`\x1b[?1004l`) при выходе. Иначе ответ терминала попал бы в эхо cooked-режима и напечатался бы в строке ввода как `^[[I`, сдвигая рамку. На время `cooked_mode()` (печать статики через `run_in_terminal`) режим приостанавливается, состояние фокуса при этом сохраняется.

`ui/notifications.py` — `notify_turn_finished(elapsed, cancelled=)`: системный тост о завершении обычного или прерванного хода, если включено `notifications_enabled`, ход длился ≥ 60 сек (`MIN_TURN_SECONDS`), терминал не в фокусе (`None` трактуется как «не в фокусе») и с прошлого уведомления прошло ≥ 5 сек. Ожидание `poll` уведомление не вызывает. Доставка — фоновым потоком-демоном: Linux `notify-send -a necli`, macOS `osascript`, Windows PowerShell toast через Windows Runtime API. Вызовы — `commands/interactive.py` после `_run_with_interrupt` (обычный ход и авто-резюм фоновой задачи). Переключатель — `/settings` → Interface (при включении кидает пробный тост).
