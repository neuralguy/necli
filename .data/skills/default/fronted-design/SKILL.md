...
## ⚠️ Главное правило: сначала скелет, потом кожа

Жёсткое правило порядка действий, нарушение которого — главная причина AI-slop:

**1. Сначала думай.** Определи смысл секции, её содержание, иерархию информации. Без этого CSS — пустое украшение.

**2. Потом пиши HTML.** Только семантическая разметка: заголовки, параграфы, списки, секции. Ни одного CSS-класса, ни одного `style=`. Разметка должна работать и выглядеть осмысленно даже без стилей — как в читалке или в текстовом браузере.

**3. Только затем — CSS.** Когда структура готова, содержимое написано, иерархия выстроена — добавляй стили. Шрифты, цвета, отступы, анимации — всё это декорация поверх готового скелета.

**Почему это важно:** AI-агенты по умолчанию пишут HTML и CSS одновременно, вперемешку. В результате структура подстраивается под визуал, а не под содержание. Сайт может выглядеть красиво, но:
- заголовки не отражают иерархию
- секции не имеют смысла без стилей
- содержимое страдает ради «красивого макета»

**Проверка:** выдели весь текст страницы — остаётся ли связная статья? Если да — ты сделал правильно. Если нет — переделывай, начиная с HTML.
...
## 3. Антипаттерны-ловушки (запрещённые дефолты и палитры)

> Этот раздел — не про «как красиво», а про «что уже заезжено AI-агентами до тебя». Если в твоём дизайне есть хоть один из этих паттернов без осознанного обоснования — переделывай.

### 3.1 The Lila Rule — запрет AI-фиолетового

«AI Purple / Blue glow» — самый узнаваемый признак того, что дизайн сделал LLM без контроля.

**Абсолютно запрещено как дефолт:**
- Фиолетовый градиент на hero вместо визуального контента
- Фиолетовое свечение кнопок «по умолчанию»
- Neon-фиолетовые градиенты без брендового обоснования
- Сочетание «тёмная тема + сиреневый акцент» — самое частотное AI-клише

**Когда можно:** бренд явно базируется на фиолетовом (Twitch, Zendesk, Monzo). Но выполни с intent: согласованная палитра, приглушённые нейтральные, сдержанные градиенты. Не «generic AI gradient slop».

**Что делать вместо:** нейтральная база (Zinc/Stone/Slate) + один яркий акцент (Emerald, Electric Blue, Deep Rose, Burnt Orange).

### 3.2 Шрифтовые ловушки — запрещённые дефолты

**Inter как sans «по умолчанию»** — самое частое AI-клише 2024-2026. Inter — отличный шрифт, но его бездумное использование создаёт ощущение шаблонности.

**Правило:** Inter можно ТОЛЬКО если:
- Бренд явно использует Inter (Linear, Vercel)
- Проект public-sector / accessibility-first (высокая читаемость)
- Inter идёт в паре с характерным display-напарником

**Запрещены как дефолт:**
- **Fraunces** — самый популярный LLM-display serif. Не используй без явного брендового обоснования.
- **Instrument Serif** — то же самое, AI-маркер.

**Что делать вместо display:**
- Sans: Geist, Satoshi, Cabinet Grotesk, ABC Diatype, PP Neue Montreal, GT Walsheim, Migra Sans
- Serif (только при обосновании): PP Editorial New, GT Sectra Display, Cardinal Grotesque, Reckless Neue, Tiempos Headline, Recoleta

### 3.3 Serif Discipline — когда serif уместен

Serif в заголовках — самый распространённый приём, которым AI пытается добавить «премиумности». Чаще всего — безосновательно.

**Serif допустим ТОЛЬКО когда одно из:**
- Бренд-гайд явно называет конкретный serif
- Эстетика проекта — редакционная / лакшери / издательская / историческая, **И** ты можешь объяснить, почему именно этот serif подходит этому бренду

**Для всего остального** (креативное агентство, студия дизайна, modern brand, портфолио, lifestyle, SaaS) — sans-serif display по умолчанию. Sans-заголовки — не «скучно», это тот же стандарт, что чёрный цвет в моде.

**Mixed-family emphasis — запрещён:**
- ❌ «Мы создаём *Brand*» (serif слово в sans-заголовке) — любительский приём
- ✅ «Мы создаём **Brand**» (bold того же шрифта) — профессионально
- ✅ «Мы создаём *Brand*» (italic того же шрифта) — профессионально

**Italic descender clearance (обязательно):**
Когда italic используется в display-заголовке и слово содержит буквы с нижними выносными элементами (у, р, з, д, q, j, p, g), `leading: 1` или `leading-none` обрежет эти элементы.
```css
/* Всегда при italic в display */
.italic-headline {
  line-height: 1.1;    /* не leading-none */
  padding-bottom: 0.25rem;  /* запас */
}
```
Проверь КАЖДОЕ italic-слово в display-заголовках перед сдачей.

### 3.4 Premium-consumer palette ban

Для премиум-потребительских брендов (посуда, wellness, artisan, luxury, DTC) LLM по умолчанию генерирует тёплую бежево-кремовую палитру с латунными/терракотовыми акцентами.

**Запрещённые hex-семейства как дефолт:**

| Тип | Запрещённые тона | Примеры |
|-----|-----------------|---------|
| **Фоны** | Тёплый крем/пергамент/кость | `#f5f1ea`, `#f7f5f1`, `#fbf8f1`, `#efeae0`, `#ece6db`, `#e8dfcb` |
| **Акценты** | Латунь/терракота/oxblood/охра | `#b08947`, `#b6553a`, `#9a2436`, `#9c6e2a`, `#bc7c3a`, `#7d5621` |
| **Текст** | Espresso/тёплый near-black | `#1a1714`, `#1a1814`, `#1b1814` |

**Когда можно:** бренд реально построен вокруг этих цветов (Hermès, Le Creuset). Но сделай осознанно, не как AI-дефолт.

### 3.5 Color consistency lock

**Один акцент на проект.** Тёпло-серый сайт не получает синюю CTA в седьмой секции. Розовый акцент не соседствует с бирюзовым бейджем в футере.

```css
/* ❌ Ошибка: разные акценты на одной странице */
.section-hero { --accent: oklch(0.55 0.17 30); }      /* оранж */
.section-cta { --accent: oklch(0.55 0.20 250); }      /* синий */

/* ✅ Правильно: один акцент на всё */
:root { --accent: oklch(0.55 0.17 30); }
```

**Правило:** выбрал акцент — зафиксируй. Перед сдачей проверь каждый компонент. Одна палитра на проект (не переключайся между тёплыми и холодными нейтральными).

### 3.6 Emoji policy — без emoji по умолчанию

Emoji в коде, разметке и видимом тексте — запрещены по умолчанию. Заменяй символами из иконочных библиотек.

**Исключение:** когда пользователь явно запросил playful/чат-стайл/social-native вайб — и даже тогда используй их сдержанно и осознанно.
...
<!-- ✅ Хорошо -->
<button><svg icon="mail" aria-hidden="true">...</svg> Отправить</button>
<span class="icon"><svg icon="warning" aria-hidden="true">...</svg></span>
```

### 3.7 Icon library priority

Иконочные библиотеки — с приоритетом, не любую подряд.

```
🥇 @phosphor-icons/react — рекомендуемый стандарт
🥇 hugeicons-react — альтернатива
🥇 @radix-ui/react-icons — для UI-компонентов
🥈 @tabler/icons-react — допустимо
❌ lucide-react — не рекомендуется (только если проект уже на нём)
```

**Правила:**
- Одна библиотека на проект, не смешивай Phosphor с Lucide
- Стандартизируй `strokeWidth` глобально (1.5 или 2.0)
- Никогда не рисуй SVG-иконки руками — если glyph нет, доустанови библиотеку

---

## 4. Spacing и композиция
...
## 7. Моушн и микро-взаимодействия
...
### 7.3 Микро-паттерны (copy-paste готовые)
...
### 8.1 WCAG 2.2 AA — обязательный минимум
...
### 8.5 Цвет не единственный носитель
...
### 8.7 ARIA — когда семантики не хватает
...
## 10. Контент и структура страницы

### 10.1 Что делает Hero работающим
...
### 10.2 Структура секций (антишаблоны)
...
### 10.3 Hero без картинки (для dev-тулов)
...
### 11.2 Responsive layout patterns
...
## 12. Детали, которые читаются как «дорого»
...
## 13. Полный чеклист перед сдачей
...
## 14. Аудит существующего проекта
...
## 18. Формы — полный набор состояний

### 18.1 Структура формы
...
## 19. Дата-визуализация / Дашборды

### 19.1 Статистические карточки
...
### 19.2 Прогресс-бар (множественный)
...
## 20. Анимации готовые (copy-paste)

### 20.1 Stagger — поочередное появление
...
### 20.3 Появление секции при скролле
...
## 21. E-commerce паттерны

### 21.1 Карточка товара
...
## 22. Типовые секции и альтернативы шаблонам

### 22.1 Вместо «3 карточек features» — покажи продукт в действии
...
### 22.2 Вместо «How it works 3 шага» — убери банальности
...
### 22.3 Вместо «Testimonials» — кейс с цифрами
...
### 22.4 Hero с убеждением — не «AI-powered platform», а конкретика
...
## 23. Конкретные сценарии по типу продукта
...
## 25. Производительность (Core Web Vitals)
...
## 27. Характер в дизайне: практические приёмы
...
### 27.1 Визуальный язык = система правил, а не набор токенов
...
### 27.2 Секции должны быть разными (Background variation)
...
### 27.3 Тактильные / физические элементы
...
### 27.4 Бренд-нейминг и копирайтинг как часть дизайна
...
### 27.5 WOW-элемент: тот самый «chef's kiss»
...
6. **Градиентный orb на фоне** — живое пятно, медленно меняющее позицию (смотри раздел 4.3)
...
### 27.6 Конкретная ошибка: разбор Before/After
...
### 27.7 Landing page: минимальная структура, которая работает
